...
public class PaymentController {
  ...
  public PaymentResponseDto post(@RequestBody @Valid final PaymentRequestDto paymentRequestDto) {
    ...
    final Optional<BuyerEntity> opBuyer = this.buyerBusiness.findById(paymentRequestDto.getBuyer().getId());
    final BuyerEntity buyer = opBuyer.orElseThrow(() ->
        new EntityNotFoundException("Buyer"));
    
    final Optional<ClientEntity> opClient = this.clientBusiness.findById(paymentRequestDto.getClient().getId());
    ...
    if (paymentRequestDto.getCreditCard() != null) {
      opCreditCard = this.creditCardBusiness.findById(
          paymentRequestDto
            .getCreditCard()
            .getId());
